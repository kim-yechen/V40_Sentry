...
class QuantumControlCenter:
    ...
    def load_resource(self, file_name):
        ...
        mapping = {
            ...
            "COMMODITY": "COMMODITY_ANALYSIS_REPORT.xlsx",
            "NGX_DATA": "V40_NGX_100_COMPLETE (1).xlsx",
            "NBI_DATA": "V40_NBI_260_COMPLETE (1).xlsx",
            "MINING_DATA": "V7C_GLOBAL_MINING_TOTAL_REPORT_20260116.xlsx"
        }
        path = mapping.get(file_name, file_name)
        if not os.path.exists(path):
            self.error_log.append(f"❌ [자원실종] {path}")
            return None
        try:
    ...
    def process_macro(self):
        ...
        try:
            ...
            if vix >= 25:
                ...
                # 2. V8 리소스 기반 비중 결정
                v8_df = self.load_resource("V8_REVISION_FINAL")
                if v8_df is not None:
                    target_col = next((c for c in v8_df.columns if any(x in str(c) for x in ['NextGen', 'V8', 'Cash'])), None)
                    raw_v8 = v8_df[target_col].dropna().iloc[-1] if target_col else 18.0
                    self.v8_p = float(raw_v8) * 100 if float(raw_v8) <= 1.0 else float(raw_v8)
            ...
            self.error_log.append(f"공정 1 내부 모순: {e}")
            return True
    # --------------------------------------------------------------------------
...
def process_macro(self):
    ...
    # --------------------------------------------------------------------------
def process_floor_2(self):
        logging.info("공정 3: 2층 전략주(NGX/NBI/BNAI/Mining) 통합 가공")
        try:
            f2_data = []
            res_map = {
                "🚀 [NGX-3]": "NGX_DATA", "🧬 [NBI-3]": "NBI_DATA",
                "🚀 [TEN-B]": "BNAI_DATA", "💎 [MINING-P]": "MINING_DATA"
            }
            
            for sec, res_key in res_map.items():
                df = self.load_resource(res_key)
                if df is None or df.empty: continue
                
                # 섹션별 정렬 및 필터링
                sort_col = 'V_Energy' if 'V_Energy' in df.columns else ('V40_Energy' if 'V40_Energy' in df.columns else 'Energy')
                top_3 = df.sort_values(by=sort_col, ascending=False).head(3)
                
                for _, r in top_3.iterrows():
                    sym = r.get('Ticker', r.get('Symbol', 'N/A'))
                    en = r.get(sort_col, 0.0)
                    label = f"{sym}(E:{en:.1f})"
                    self.sections[sec].append(label)
                    f2_data.append({"Section": sec, "Ticker": sym, "Energy": en})
            
            self.floor_2_df = pd.DataFrame(f2_data)
            return not self.floor_2_df.empty
        except Exception as e:
            self.error_log.append(f"2층 공정 모순: 형님, 수식 수정 요망 ({e})")
            return True
